...
class HierarchicalAggregation(Function):
    ...
    def forward(ctx, semantic_label, coord_shift, ball_query_idxs, start_len, batch_idxs, training_mode,
        ...
        ops.hierarchical_aggregation(semantic_label, coord_shift, batch_idxs, ball_query_idxs, start_len,
...
def ball_query(coords, batch_idxs, batch_offsets,
                         adj_mat_inst,similarity_threshold_inst,
                         adj_mat_para, similarity_threshold_para,
                         radius, mean_active, with_octree=False):
    if with_octree:
        return octree_ball_query(coords, mean_active, radius)
    else:
        return ballquery_batch_p(coords, batch_idxs, batch_offsets,
                                 adj_mat_inst,similarity_threshold_inst,
                         adj_mat_para, similarity_threshold_para, radius, mean_active)



def ball_query_easy(coords, batch_idxs, batch_offsets,
                         radius, mean_active, with_octree=False):
    return ballquery_batch_p_easy(coords, batch_idxs, batch_offsets, radius, mean_active)
...
#     @staticmethod
#     def forward(ctx, coords, batch_idxs, batch_offsets,
#                 adj_mat_para, similarity_threshold_para,
#                 radius, meanActive):
#     # def forward(ctx, coords, batch_idxs, batch_offsets,
#     #         radius, meanActive):
...
#         assert batch_idxs.is_contiguous() and batch_idxs.is_cuda
#         assert batch_offsets.is_contiguous() and batch_offsets.is_cuda
#         assert adj_mat_para.is_contiguous() and adj_mat_para.is_cuda

#         while True:
#             idx = torch.cuda.IntTensor(n * meanActive).zero_()
#             start_len = torch.cuda.IntTensor(n, 2).zero_()
#             nActive = ops.ballquery_batch_p(coords, batch_idxs, batch_offsets,
#                                             adj_mat_para, similarity_threshold_para,
#                                             idx, start_len, n,
#                                             meanActive, radius)
#             # nActive = ops.ballquery_batch_p(coords, batch_idxs, batch_offsets, idx, start_len, n,
...
class BallQueryBatchP(Function):
    ...
    def forward(ctx, coords, batch_idxs, batch_offsets,
        ...
        assert batch_idxs.is_contiguous() and batch_idxs.is_cuda
        assert batch_offsets.is_contiguous() and batch_offsets.is_cuda
        assert adj_mat_inst.is_contiguous() and adj_mat_inst.is_cuda
        assert adj_mat_para.is_contiguous() and adj_mat_para.is_cuda
...
class BallQueryBatchP_Easy(Function):
    ...
    def forward(ctx, coords, batch_idxs, batch_offsets,
        ...
        while True:
            idx = torch.cuda.IntTensor(n * meanActive).zero_()
            start_len = torch.cuda.IntTensor(n, 2).zero_()
            nActive = ops.ballquery_batch_p_easy(coords, batch_idxs, batch_offsets,
                                            idx, start_len, n,
                                            meanActive, radius)
            # nActive = ops.ballquery_batch_p(coords, batch_idxs, batch_offsets, idx, start_len, n,
            #                                 meanActive, radius)
    ...
    def backward(ctx, a=None, b=None):
        return None, None, None    
ballquery_batch_p_easy = BallQueryBatchP_Easy.apply





# class BallQueryBatchP_para_aware(Function):
...
#             idx = torch.cuda.IntTensor(n * meanActive).zero_()
#             start_len = torch.cuda.IntTensor(n, 2).zero_()
#             nActive = ops.ballquery_batch_p_para_aware(coords, batch_idxs, batch_offsets,
#                                             adj_mat_para, similarity_threshold_para,
#                                             idx, start_len, n,
#                                             meanActive, radius)
#             # nActive = ops.ballquery_batch_p(coords, batch_idxs, batch_offsets, idx, start_len, n,
...
# ballquery_batch_p_para_aware = BallQueryBatchP_para_aware.apply
# ballquery_batch_p_inst_pare = BallQueryBatchPWithInstAndPare.apply
