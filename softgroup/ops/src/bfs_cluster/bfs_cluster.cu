...
__global__ void ballquery_batch_p_cuda_(int n, int meanActive, float radius,
                                        ...
                                        const float *adj_mat_inst,float similarity_threshold_inst,
                                        const float *adj_mat_para, float similarity_threshold_para,
                                        int *idx,
  ...
  for (int k = start; k < end; k++) {
    ...
    if ((d2 < radius2
    && (adj_mat_inst[pt_idx * n + k] > similarity_threshold_inst)&& (adj_mat_para[pt_idx * n + k] > similarity_threshold_para) ) )
    // if (adj_mat_inst[pt_idx * n + k] > similarity_threshold_inst
    // || adj_mat_para[pt_idx * n + k] > similarity_threshold_para) 
    {
      //3000
      if (cnt < 3000) {
        idx_temp[cnt] = k;
...
int ballquery_batch_p_cuda(int n, int meanActive, float radius,
  ...
  ballquery_batch_p_cuda_<<<blocks, threads, 0, stream>>>(
      n, meanActive, radius, xyz, batch_idxs, batch_offsets,
      adj_mat_inst, similarity_threshold_inst,adj_mat_para, similarity_threshold_para, idx, start_len,
      p_cumsum);
