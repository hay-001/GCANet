...
class QueryDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.out_cls = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, num_class))
        self.out_score = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, 1))
        self.out_paras = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, 22))
    ...
    def prediction_head(self, query, mask_feats, batch_offsets):
        ...
        pred_paras = self.out_paras(query)
        pred_masks, attn_masks = self.get_mask(query, mask_feats, batch_offsets)
        return pred_labels, pred_scores, pred_masks, pred_paras, attn_masks
    ...
    def forward_simple(self, x, batch_offsets):
        ...
        pred_labels, pred_scores, pred_masks,pred_paras, _ = self.prediction_head(query, mask_feats, batch_offsets)
        return {'labels': pred_labels,'parameters': pred_paras, 'masks': pred_masks, 'scores': pred_scores}
    ...
    def forward_iter_pred(self, x, batch_offsets):
        ...
        if getattr(self, 'pe', None):
            ...
            pe = None
        pred_labels, pred_scores, pred_masks,pred_paras,attn_masks = self.prediction_head(query, mask_feats, batch_offsets)
        prediction_labels.append(pred_labels)
        prediction_scores.append(pred_scores)
        prediction_masks.append(pred_masks)
        prediction_paras.append(pred_paras)
        for i in range(self.num_layer):
