...
class KPAM(nn.Module):
    ...
    def forward(self, x, attention_feature):
        attention_feature = attention_feature.squeeze(2).permute(0, 2, 1)
        attention_feature = self.conv1(attention_feature).permute(0, 2, 1)  # b,c/latent,n
        attention_feature = self.sigmoid(attention_feature).unsqueeze(-1)  # b,n,n
...
class OFFSET_PRED_MODULE(nn.Module):
    def __init__(self, nn_nb=60, sampling_ratio=120):
        ...
        self.attention_seg = KPAM(nn_nb)
        self.attention_inst = KPAM(nn_nb)

        self.mlp_offset = torch.nn.Conv1d(256, 3, 1)

    def forward(self, points, feature, semantic_feature, instance_feature, index):
        # print(points.shape)
        # print(feature.shape)
        ...
        feature_sampling = torch.gather(feature, 1, subidx.repeat(1, 1, feature.shape[2]))  # [b, 6, n_sub]
        # print(feature_sampling.shape)

        # feature_sampling=self.transformer(feature_sampling)

        key_points = torch.gather(points, 1, subidx.repeat(1, 1, points.shape[2]))  # [b, 3, n_sub]
        # print(key_points.shape)
        key_points = key_points.permute(0, 2, 1).contiguous()
        points = points.permute(0, 2, 1).contiguous()
        feature_sampling = feature_sampling.permute(0, 2, 1).contiguous()

        points_knn, feature_knn, idx = group_points(
            self.k, point_cloud=key_points,
            query_cloud=points,
            point_features=feature_sampling)
        # print(points_knn.shape)
        # print(feature_knn.shape)
        # print(idx.shape)

        semantic_feature = semantic_feature.permute(0, 2, 1).contiguous()
        instance_feature = instance_feature.permute(0, 2, 1).contiguous()

        semantic_feature_knn = grouping_operation(semantic_feature, idx)
        # print(semantic_feature_knn.shape)

        instance_feature_knn = grouping_operation(instance_feature, idx)
        # print(instance_feature_knn.shape)

        # point_cloud (torch.Tensor): 点云数据, shape [B, N, C]
        # neighborhood (torch.Tensor): 每个点的K个邻域点的数据, shape [B, N, K, C]

        semantic_feature_knn = semantic_feature_knn.permute(0, 2, 3, 1)
        instance_feature_knn = instance_feature_knn.permute(0, 2, 3, 1)


        # print(points.shape)
        # print(points_knn.permute(0, 2, 3, 1).shape)
        # print(points.permute(0, 2, 1).unsqueeze(2).expand(-1, -1,self.k, -1).shape)

        points_knn_direction=points_knn.permute(0, 2, 3, 1)-points.permute(0, 2, 1).unsqueeze(2).expand(-1, -1,self.k, -1)
        # print(points_knn.permute(0, 2, 3, 1)[0,0,0:4,:])

        # print(points.permute(0, 2, 1).unsqueeze(2).expand(-1, -1,self.k, -1)[0,0,0:4,:])
        # print(points_knn_direction[0,0,0:4,:])

        # print(feature_knn.shape)
        # print(points_knn_direction.shape)

        feature_knn = feature_knn.permute(0, 2, 3, 1)
        feature_knn_plus_direction = torch.cat([feature_knn, points_knn_direction], 3)

        semantic_feature = semantic_feature.permute(0, 2, 1)
        instance_feature = instance_feature.permute(0, 2, 1)

        distances_semantic, distances_instance = inst_and_seg_dist(semantic_feature, semantic_feature_knn,
                                                                   instance_feature, instance_feature_knn)

        # feature_knn_s = self.attention_seg(feature_knn, distances_semantic)
        feature_knn_s_i = self.attention_inst(feature_knn_plus_direction, distances_instance)

        # print('feature_knn_s_i',feature_knn_s_i.shape)

        feature_knn_s_i = self.conv1(feature_knn_s_i.permute(0, 3, 2, 1))

        # print('feature_knn_s_i',feature_knn_s_i.shape)

        feature_s_i = feature_knn_s_i.max(dim=-2, keepdim=False)[0]
        # print('feature_s_i',feature_s_i.shape)

        feature_s_i = torch.cat([feature_s_i, feature.permute(0, 2, 1)], dim=1)
        # print('feature_s_i',feature_s_i.shape)

        offsets = self.mlp_offset(feature_s_i)
...
class PrimitivesEmbeddingDGCNGn(nn.Module):
    ...
    def __init__(self, opt, emb_size=64, num_primitives=10, primitives=True, embedding=True, parameters=True, mode=5,
        ...
        self.offset_pred_block = OFFSET_PRED_MODULE(nn_nb=60, sampling_ratio=120)  # 40,120
    ...
    def forward_train(self, points, normals, batch_idxs):
        ...
        if self.opt.ablation!=True:
            pt_offsets = self.offset_pred_block(points[:, 0:3, :].permute(0, 2, 1), output_feats_puls_coods,
                                                type_per_point_forgroup, output_feats, _)
        ...
        output_feats_reshape = torch.cat([output_feats, type_per_point_forgroup, param_per_point, points.permute(0, 2, 1)],
                                 dim=-1)
        output_feats_reshape = F.dropout(F.relu(
            self.bn_normal_squeeze_output_feature(self.mlp_squeeze_output_feature(output_feats_reshape.permute(0, 2, 1)))),
                                 self.drop)
        output_feats_reshape = output_feats_reshape.permute(0, 2, 1)

        output_feats_reshape = torch.reshape(output_feats_reshape, (-1, output_feats_reshape.shape[-1]))
    ...
    def forward_grouping(self,
        ...
        radius = 0.03  # 0.03
        similarity_threshold_inst = 0.995 # 0.94 ,0.995
