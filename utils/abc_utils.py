...
def forward_pass_open_spline(
    ...
    for b in range(batch_size):
        ...
        # # new_points = torch.inverse(RS[b].to('cpu')).to('cpu') @ torch.transpose(temp, 1, 0)
        # new_points = new_points.to('cuda:0')
        # print(temp.shape)
        # print(RS[b].shape)


        # RS_cpu=RS[b].cpu()
        # RS_cpu=torch.inverse(RS_cpu)

        # RS_cg = RS_cpu.to('cuda:0')


        # new_points = RS_cg @ torch.transpose(temp, 1, 0)
        new_points = torch.inverse(RS[b]) @ torch.transpose(temp, 1, 0)
...
def construction_affinity_matrix_type_one_class(inputs_xyz, type_per_point, T_param_pred,primitive_type, sigma=1.0):
    '''
    inputs_xyz: (B, 3,N)
    embedding: (B, N, 256)
    param_per_point: (B, N, 22)
    ...
    for b in range(T_pred.shape[0]):
        # t = torch.bincount(T_pred[b]).argmax()
        #
        # primitive_type = int(t)
        inputs_xyz_sub = (inputs_xyz[b].T)
        inputs_xyz_ = inputs_xyz[b].T
        if primitive_type ==6 or primitive_type ==7:
            primitive_type=9
        if primitive_type ==8:
            primitive_type=2


        if primitive_type == -1 or primitive_type == 0 or N < 30:
            continue
        if primitive_type == 2:  # process open spline
            weights = torch.ones([inputs_xyz_sub.shape[0], 1]).to(inputs_xyz_sub.device)
            reconst_points = fitter.forward_pass_open_spline(inputs_xyz_sub, weights=weights)
            ...
            param_pred = reconst_points
            distance = routines[primitive_type](points=inputs_xyz_, params=param_pred)
            distance = distance[:, None].repeat(1, N)
        elif primitive_type == 9:  # process closed spline
            weights = torch.ones([inputs_xyz_sub.shape[0], 1]).to(inputs_xyz_sub.device)
            try:
                reconst_points = fitter.forward_pass_closed_spline(inputs_xyz_sub, weights=weights,
            ...
            param_pred = T_param_pred[b]  # (M, 22)
            param_pred = param_pred[:, param_list[primitive_type][0]:param_list[primitive_type][1]]
            distance = routines[primitive_type](points=inputs_xyz_, params=param_pred)

        distance_matrix[b, :, :] = distance
    ...
    # min_dist = affinity_matrix.min()
    # max_dist = affinity_matrix.max()
    # affinity_matrix = (affinity_matrix - min_dist) / (max_dist - min_dist)
    # affinity_matrix = torch.exp(-affinity_matrix ** 2 / (2 * sigma * sigma))
...
#
 # 将对角线上的元素设置为0
    adjacency = affinity_matrix - torch.diag_embed(torch.diagonal(affinity_matrix, dim1=-2, dim2=-1))
    # # 将每行元素归一化到[0,1]范围内
    # min_dist = adjacency.min(dim=-1, keepdim=True)[0]
    # max_dist = adjacency.max(dim=-1, keepdim=True)[0]
    # adjacency = (adjacency - min_dist) / (max_dist - min_dist)
    ...
    # 将距离值转换为相似性概率
    adjacency = torch.exp(-adjacency ** 2 / (2 * 1.0 ** 2))
    affinity_matrix = adjacency - torch.diag_embed(torch.diagonal(adjacency, dim1=-2, dim2=-1))
